...
class QANet(nn.Module):
    def __init__(self, n_head=4, embedding_dim=50, feature_concat='l_r', d_k=64, attention=True, gate=None):
        super().__init__()
        ...
        if self.attention:
            ...
            in_channel = embedding_dim
        self.position_embedding = nn.Embedding(24, embedding_dim)

        self.linear_1 = nn.Linear(in_channel * 24, in_channel)
        self.linear_2 = nn.Linear(in_channel, 1)

        self.linear_3 = nn.Linear(in_channel * 24, in_channel)
        self.linear_4 = nn.Linear(in_channel, 1)

        # self.linear_1 = nn.Linear(in_channel * 24 * 2, 2)

    # def forward(self, L, R, pos):
    def forward(self, L, R):
        # L_pos = self.position_embedding(pos)
        # R_pos = self.position_embedding(pos)
        # L = L + L_pos
        # R = R + R_pos
        if self.attention:
            # out:(bs,T,embedding_dim),qkv:(bs,T,n_head*d_k),attention:(bs, n_head, T, T)
