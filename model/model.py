...
def get_pad_mask(seq, pad_idx):
    ...
    seq_len = seq.shape[1]
    mask = torch.triu(torch.ones(seq_len, seq_len), diagonal=1).byte().unsqueeze(0).unsqueeze(0)
    print(mask.shape)
    return mask
...
class QANet(nn.Module):
    def __init__(self, n_head=4, embedding_dim=50, feature_concat='l_r', d_k=64, attention=True, gate=None, position_embed=False, time_len=24, dis_fcn=False, attn_mask=False):
        ...
        if self.dis_fcn:
            self.linear_1 = nn.Linear(in_channel * time_len, in_channel*2)
            self.linear_2 = nn.Linear(in_channel*2, in_channel)

            self.linear_3 = nn.Linear(in_channel * time_len, in_channel*2)
            self.linear_4 = nn.Linear(in_channel*2, in_channel)
        else:
            self.linear_1 = nn.Linear(in_channel * time_len * 2, in_channel*2)
            self.linear_2 = nn.Linear(in_channel*2, in_channel*2)

    def forward(self, L, R, L_target, R_target):
        L = self.number_embed(L)
        R = self.number_embed(R)
        L_target = self.number_embed(L_target)
        R_target = self.number_embed(R_target)
        ...
        return L_out, R_out, L_target, R_target
...
if __name__ == '__main__':
    ...
    model = QANet(n_head=4, embedding_dim=50, feature_concat='l_out_direct', d_k=64, attention=True, gate=None)
