...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (pm25, pm10, pm25_target_value, pm10_target_value) in enumerate(self.data_loader):
            pm25, pm10, pm25_target_value, pm10_target_value = pm25.to(self.device), \
                                                               pm10.to(self.device), \
                                                               pm25_target_value.to(self.device), \
                                                               pm10_target_value.to(self.device)
            ...
            pm25_predict_embed, pm10_predict_embed, pm25_target_embed, pm10_target_embed = self.model(
                pm25, pm10, pm25_target_value, pm10_target_value)
            ...
            # pm10_cross_entropy = self.CrossEntropyLoss(pm10_p, pm10_target_value)
            pm25_embed_mse = self.loss(pm25_predict_embed, pm25_target_embed)
            pm10_embed_mse = self.loss(pm10_predict_embed, pm10_target_embed)
            mse_loss = pm25_embed_mse + pm10_embed_mse

            pm25_predict_value, pm25_target_value, pm10_predict_value, pm10_target_value = pm25_predict_value.float(), \
                                                                   pm25_target_value.float(), \
                                                                   pm10_predict_value.float(), \
                                                                   pm10_target_value.float()
            # print(pm25_predict_value==pm25_target_value, pm10_predict_value==pm10_target_value)

            pm25_loss = self.loss(pm25_predict_value, pm25_target_value)
            pm10_loss = self.loss(pm10_predict_value, pm10_target_value)
            ...
            pm25_metrics = self.metrics(pm25_predict_value, pm25_target_value)
            pm10_metrics = self.metrics(pm25_predict_value, pm10_target_value)
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, (pm25, pm10, pm25_target_value, pm10_target_value) in enumerate(self.valid_loader):
                pm25, pm10, pm25_target_value, pm10_target_value = pm25.to(self.device), \
                                                       pm10.to(self.device), \
                                                       pm25_target_value.to(self.device), \
                                                       pm10_target_value.to(self.device)
                ...
                # pm25_cross_entropy = self.CrossEntropyLoss(pm25_p, pm25_target_value)
                # pm10_cross_entropy = self.CrossEntropyLoss(pm10_p, pm10_target_value)
                pm25_embed_mse = self.loss(pm25_predict_embed, pm25_target_embed)
                pm10_embed_mse = self.loss(pm10_predict_embed, pm10_target_embed)
                mse_loss = pm25_embed_mse + pm10_embed_mse
